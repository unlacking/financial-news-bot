  ...
  [ news_collector.py ] & [ price_collector.py ]
   ...
   [ main.py / Orchestrator ]
         ...
         └── 4. Outbound Alert & Report Delivery
                ├── [ alert_engine.py ] --> Evaluates risk metrics & outputs structured alert dictionaries
                ├── [ formatter.py ]    --> Builds clean ASCII alert layouts (no redundant fields) & <3800 char digests
...
* **Rule-Based Risk & Alert Engine:** Evaluates price volatility limits, high-severity negative news, and sensitive keyword watchlists to generate structured emergency alerts.
...
* `src/alert_engine.py` — Rule-based volatility matrix, sentiment thresholds, and structured alert dictionary generator.
...
* `src/formatter.py` — Parametric ASCII alert builder and multi-chunk digest splitter.
...
SENSITIVE_KEYWORDS=điều tra,bắt giam,vỡ nợ,phá sản,thanh tra,vi phạm,bán giải chấp
...
Ensure Docker Desktop is running on your host workstation.

```bash
# Build the container image
docker compose build --no-cache

# Run the pipeline service in the background
docker compose up -d
